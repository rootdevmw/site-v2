...
import { getPublicHomecells } from "@/lib/public-api/homecells";
import { Pagination } from "@/components/public/Pagination";

const PAGE_SIZE = 9;

export default async function PublicHomecells({
  searchParams,
}: {
  ...
  const res = await getPublicHomecells({ page, limit: PAGE_SIZE });
  const homecells = res.data;
          ...
          <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="font-serif text-2xl font-semibold text-white sm:text-3xl">
                Pray, grow, and belong
              </h1>
              <p className="mt-1.5 text-sm text-[#e6c79c]">
                Find a place where you can encourage each other in a community
                driven by the will of God.
              </p>
            </div>
            {total > 0 && (
              <p className="mt-3 text-xs font-medium text-[#c2a23a] sm:mt-0">
                {total} homecell{total !== 1 ? "s" : ""}
              </p>
            )}
          </div>
        </div>
          ...
          {homecells.length > 0 ? (
            ...
            <div className="rounded-2xl border border-[#e8c49a] bg-white px-8 py-20 text-center">
              ...
              <p className="mt-2 text-sm text-[#6b4c2a]">
                Check back soon — new homecells you can join will be added
                regularly.
              </p>
            </div>
          )}
        ...
        <div className="mx-auto max-w-screen-2xl flex flex-col items-start gap-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-[15px] font-medium text-[#4a2008]">
              There is a place for you — where you are, brothers and sisters are
              near you to share the glory of Christ together.
            </p>
          </div>
